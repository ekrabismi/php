<?php
	...
	$_SESSION['last_page'] = 'message-sent.php';
	
	require_once('includes/header.php');
	
	$message_header_title = 'Sent Messages';
	
	$query_get_message = 'SELECT message.* FROM message, message_sender WHERE message.message_id = message_sender.message_id AND message_sender.sender = "'.$username.'" AND message_sender.status = "sent" ORDER BY message_date DESC';
...
<div class="content">
	...
	<div class="content-body"><?php
		...
		if($message_count > 0) {
				...
				<tbody><?php
					while($row_get_message = mysql_fetch_assoc($result_get_message)) {
						...
						?><tr>
							...
							<td><a href="process.php?t=<?php echo encrypt('message_sent'); ?>&m=<?php echo $message_id_encrypted; ?>" title="Delete Message"><img src="images/message-delete.png" title="Delete Message" class="ttTipsy" /></a></td>
						</tr>
			...
			?><p>You have no sent messages.</p><?php
